...
export function DashboardHeader({
  ...
  return (
    <motion.header
      ...
      className="fixed top-0 left-0 right-0 flex justify-between items-center w-full px-4 md:px-6 py-0 h-16 bg-white/80 dark:bg-[#0a0a0a]/90 backdrop-blur-xl border-b border-[#e4e4e7] dark:border-[#1a1a1a] z-[60] shrink-0 font-['Sora']"
    >
      <div className="flex items-center gap-3 lg:gap-5">
        {onMenuClick && (
          <motion.button
            onClick={onMenuClick}
            className="lg:hidden p-2 text-[#71717a] dark:text-[#a1a1aa] hover:bg-[#f4f4f5] dark:hover:bg-[#1a1a1a] hover:text-[#09090b] dark:hover:text-white rounded-xl transition-all duration-200"
            whileTap={{ scale: 0.95 }}
          >
        ...
        <div>
          <h2 className="font-semibold text-sm tracking-tight text-[#09090b] dark:text-white whitespace-nowrap">{title}</h2>
          {subtitle && <p className="text-[10px] text-[#71717a] dark:text-[#a1a1aa] uppercase tracking-[0.18em] hidden md:block mt-0.5">{subtitle}</p>}
        </div>
        {search && (
      ...
      <div className="flex items-center gap-1 sm:gap-3">
        <div className="flex items-center gap-1 pr-2 sm:pr-3 border-r border-[#e4e4e7] dark:border-[#1a1a1a]">
          <ThemeToggle />
          <div className="relative" ref={notifRef}>
            <motion.button
              onClick={() => { setNotifOpen(!notifOpen); setProfileOpen(false); }}
              className="p-2 hover:bg-[#f4f4f5] dark:hover:bg-[#1a1a1a] text-[#71717a] dark:text-[#a1a1aa] hover:text-[#09090b] dark:hover:text-white transition-all duration-200 relative rounded-xl"
        ...
        <div className="relative flex items-center gap-2 sm:gap-3" ref={profileRef}>
          <div className="text-right hidden sm:block">
            <p className="font-semibold text-sm tracking-tight text-[#09090b] dark:text-white leading-tight">{userName}</p>
            <p className="text-[10px] text-[#71717a] dark:text-[#a1a1aa] tracking-[0.18em] uppercase whitespace-nowrap mt-0.5">{role}</p>
          </div>
          <motion.button
            onClick={() => { setProfileOpen(!profileOpen); setNotifOpen(false); }}
            className="w-9 h-9 bg-gradient-to-br from-[#f4f4f5] to-[#e4e4e7] dark:from-[#1a1a1a] dark:to-[#111111] border border-[#e4e4e7] dark:border-[#27272a] flex items-center justify-center rounded-full hover:border-[#09090b] dark:hover:border-white transition-all duration-200 overflow-hidden"
            aria-label="User profile menu"
            aria-expanded={profileOpen}
            aria-haspopup="true"
            whileTap={{ scale: 0.95 }}
          >
            <span className="material-symbols-outlined text-[20px] text-[#71717a] dark:text-[#a1a1aa]">person</span>
          </motion.button>
